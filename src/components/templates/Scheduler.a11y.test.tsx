...
describe('Scheduler accessibility regressions', () => {
  ...
  it('exposes selected slot state with radio semantics', async () => {
    ...
    expect(
      screen.getByRole('radiogroup', { name: /available appointment slots/i }),
    ).toBeInTheDocument();
  ...
  it('does not show restored-draft notice for baseline step-1 autosave state', () => {
    window.sessionStorage.setItem(
      'kb_booking_draft_v1',
      JSON.stringify({
        version: 1,
  ...
  it('excludes inactive staff members from preferred barber options', () => {
    ...
    expect(
      screen.getByRole('option', { name: 'Any barber' }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('option', { name: 'Active Barber' }),
    ).toBeInTheDocument();
    expect(
  ...
  it('uses a unique scheduler staff control id and de-duplicates Any barber fallback options', () => {
    ...
    const anyBarberOptions = screen.getAllByRole('option', {
      name: 'Any barber',
    });
    expect(anyBarberOptions).toHaveLength(1);
    expect((preferredBarberSelect as HTMLSelectElement).options).toHaveLength(
      2,
    );
  });
});
