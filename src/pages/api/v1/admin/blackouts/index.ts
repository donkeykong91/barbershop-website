...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (req.method === 'POST') {
    ...
    if (!scope || !startsAt || !endsAt) {
      res.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'Missing blackout fields' } });
      return;
    }

    const overlap = await hasBookingOverlap({ scope, staffId, startsAt, endsAt, reason });
    if (overlap) {
      res.status(409).json({ error: { code: 'BOOKING_OVERLAP', message: 'Blackout overlaps existing confirmed bookings' } });
      return;
    }

    const created = await createBlackout({ scope, staffId, startsAt, endsAt, reason, createdBy: 'admin' });
